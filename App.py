...
# 3. Thuật toán lấy tuyến đường xe máy qua OSRM (đường thực tế)
...
# Đọc tọa độ từ tham số URL nếu JS đã bắt được GPS
...
with st.sidebar:
    ...
    # Nút bấm HTML/JS tự động bắt GPS chuẩn trên trình duyệt điện thoại
        ...
        <script>
            document.getElementById('gps_btn').onclick = function() {
                if (navigator.geolocation) {
                    this.innerHTML = "⌛ Đang lấy vị trí...";
                    navigator.geolocation.getCurrentPosition(function(position) {
                        ...
                        alert("Không thể lấy vị trí. Vui lòng cho phép quyền GPS trên thiết bị.");
                        document.getElementById('gps_btn').innerHTML = "📍 Tôi đang đứng";
    ...
    if st.button("Bấm", type=btn_type):
        if st.session_state['user_lat'] is None:
            st.error("Vui lòng bấm '📍 Tôi đang đứng' và chấp nhận chia sẻ vị trí trước!")
        else:
...
# Nếu đã bấm "Bấm" -> Tính toán và vẽ lộ trình xe máy
...
# Chưa bấm "Bấm" -> Chỉ hiển thị các Marker
else:
