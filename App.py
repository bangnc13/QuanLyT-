...
# 2. Inject CSS Custom (Chuỗi an toàn loại bỏ triệt để lỗi unterminated string)
css_styles = [
    "<style>",
    ...
    "[data-testid='stSidebarCollapseButton'] button, [data-testid='stSidebarCollapseButton'], [data-testid='stSidebarExpandButton'] button, [data-testid='stSidebarExpandButton'], button[data-testid='baseButton-headerNoPadding'], [data-testid='collapsedControl'], [data-testid='collapsedControl'] button { background-color: #1a0a00 !important; border: 2px solid #ff6600 !important; border-radius: 50% !important; color: #ff6600 !important; box-shadow: 0 0 12px rgba(255, 102, 0, 0.8) !important; transition: all 0.3s ease-in-out !important; width: 42px !important; height: 42px !important; display: flex !important; align-items: center !important; justify-content: center !important; padding: 0 !important; margin: 5px !important; }",
    "[data-testid='stSidebarCollapseButton'] button:hover, [data-testid='stSidebarCollapseButton']:hover, [data-testid='stSidebarExpandButton'] button:hover, [data-testid='stSidebarExpandButton']:hover, button[data-testid='baseButton-headerNoPadding']:hover, [data-testid='collapsedControl']:hover, [data-testid='collapsedControl'] button:hover { background-color: #ff6600 !important; color: #000000 !important; box-shadow: 0 0 25px rgba(255, 102, 0, 1) !important; transform: scale(1.1) !important; }",
    "[data-testid='stSidebarCollapseButton'] svg, [data-testid='stSidebarExpandButton'] svg, button[data-testid='baseButton-headerNoPadding'] svg, [data-testid='collapsedControl'] svg { fill: #ff6600 !important; color: #ff6600 !important; stroke: #ff6600 !important; transition: all 0.3s ease-in-out !important; }",
    "[data-testid='stSidebarCollapseButton'] button:hover svg, [data-testid='stSidebarExpandButton'] button:hover svg, button[data-testid='baseButton-headerNoPadding']:hover svg, [data-testid='collapsedControl']:hover svg, [data-testid='collapsedControl'] button:hover svg { fill: #000000 !important; color: #000000 !important; stroke: #000000 !important; }",
    "[data-testid='stSidebar'] { background: linear-gradient(180deg, #331400 0%, #1f0c00 100%) !important; border-right: 2px solid #ff6600 !important; box-shadow: 5px 0px 15px rgba(255, 102, 0, 0.4) !important; }",
    ".robot-title { font-family: 'Orbitron', sans-serif; color: #00f0ff !important; text-align: center; text-transform: uppercase; letter-spacing: 2px; text-shadow: 0 0 10px rgba(0, 240, 255, 0.7); margin-top: 15px; margin-bottom: 20px; }",
    "div.stButton > button { font-family: 'Orbitron', sans-serif !important; font-weight: 700 !important; color: #000000 !important; background: linear-gradient(135deg, #00f0ff 0%, #7000ff 100%) !important; border: none !important; border-radius: 4px !important; padding: 12px 24px !important; text-transform: uppercase !important; letter-spacing: 1.5px !important; clip-path: polygon(10% 0, 100% 0, 90% 100%, 0 100%); transition: all 0.3s ease-in-out !important; box-shadow: 0 0 15px rgba(0, 240, 255, 0.4) !important; }",
    "div.stButton > button:hover { transform: scale(1.03) translateY(-2px) !important; box-shadow: 0 0 25px rgba(0, 240, 255, 0.8), 0 0 10px rgba(112, 0, 255, 0.8) !important; color: #ffffff !important; }",
    ".hud-card { background: rgba(51, 20, 0, 0.85) !important; border: 1px solid #ff6600 !important; border-left: 4px solid #00f0ff !important; border-radius: 6px; padding: 12px; margin-top: 10px; box-shadow: inset 0 0 10px rgba(255, 102, 0, 0.2); font-family: 'Rajdhani', sans-serif; }",
...
st.markdown("\n".join(css_styles), unsafe_allow_html=True)
...
# INJECT JAVASCRIPT REALTIME MARKER VÀ NÚT TỌA ĐỘ GPS
js_script = [
    "<script>",
    "setTimeout(function() {",
    ...
    "    var googleDotIcon = L.divIcon({ className: 'gmaps-marker', html: '<div class=\"gmaps-pulse\"></div><div class=\"gmaps-dot\"></div>', iconSize: [24, 24], iconAnchor: [12, 12] });",
    "    var userMarker = null, currentLatLng = null;",
    ...
    "                if (centerMap) { map.flyTo(currentLatLng, 16, { animate: true, duration: 1.5 }); }",
    "            }, function(error) { console.error('Lỗi GPS: ', error); }, { enableHighAccuracy: true, maximumAge: 1000, timeout: 5000 });",
    "        }",
    "    }",
    ...
    "        div.innerHTML = '<button title=\"Định vị vị trí hiện tại\" style=\"background-color: #1a0a00; border: 2px solid #00f0ff; border-radius: 50%; width: 44px; height: 44px; cursor: pointer; display: flex; justify-content: center; align-items: center; box-shadow: 0 0 15px rgba(0,240,255,0.6); transition: all 0.3s ease;\"><svg width=\"22\" height=\"22\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"#00f0ff\" stroke-width=\"2.5\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><circle cx=\"12\" cy=\"12\" r=\"7\"></circle><line x1=\"12\" y1=\"2\" x2=\"12\" y2=\"5\"></line><line x1=\"12\" y1=\"19\" x2=\"12\" y2=\"22\"></line><line x1=\"2\" y1=\"12\" x2=\"5\" y2=\"12\"></line><line x1=\"19\" y1=\"12\" x2=\"22\" y2=\"12\"></line></svg></button>';",
    "        div.onclick = function(e) { e.stopPropagation(); updateLocation(true); };",
    "        return div;",
    "    };",
    ...
    "</script>",
    "<style>",
    ".gmaps-marker { position: relative; width: 24px; height: 24px; }",
    ".gmaps-dot { width: 16px; height: 16px; background-color: #4285F4; border: 3px solid #FFFFFF; border-radius: 50%; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); box-shadow: 0 2px 6px rgba(0,0,0,0.4); z-index: 2; }",
    ".gmaps-pulse { width: 40px; height: 40px; background-color: rgba(66, 133, 244, 0.35); border-radius: 50%; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); animation: gmaps-ripple 2s infinite ease-out; z-index: 1; }",
    "@keyframes gmaps-ripple { 0% { width: 16px; height: 16px; opacity: 0.8; } 100% { width: 50px; height: 50px; opacity: 0; } }",
    ".custom-gps-btn button:hover { background-color: #00f0ff !important; transform: scale(1.1); box-shadow: 0 0 25px rgba(0, 240, 255, 1) !important; }",
    ".custom-gps-btn button:hover svg { stroke: #000000 !important; }",
    "</style>"
]
