...
# 1. Cấu hình trang
st.set_page_config(
    page_title="Hệ Thống Tối Ưu Lộ Trình - GPS Live Sync",
    page_icon="🤖",
    layout="wide",
...
# 4. Thuật toán tối ưu đường đi vòng kín
...
with st.sidebar:
    ...
    st.markdown("""
    <div class='hud-card'>
        <div class='hud-label'>Trạng thái GPS</div>
        <div id='gps-status' class='hud-value' style='font-size:1rem; color:#00f0ff;'>Đang tìm tín hiệu GPS...</div>
    </div>
    """, unsafe_allow_html=True)
...
# Marker GPS Realtime & La bàn
arrow_html = """
<div id="live-user-marker" style="width: 48px; height: 48px; display: flex; justify-content: center; align-items: center; transition: transform 0.1s linear;">
    <svg width="44" height="44" viewBox="0 0 24 24" fill="none" style="filter: drop-shadow(0px 0px 8px #00f0ff);">
...
folium.Marker(
    ...
    icon=folium.DivIcon(icon_size=(48,48), icon_anchor=(24,24), html=arrow_html)
).add_to(m)
...
# 5. JAVASCRIPT REALTIME LIVE TRACKING & LA BÀN TỐC ĐỘ CAO (Client-Side Rendering)
components.html("""
<script>
    const pWindow = window.parent;
    const pDoc = pWindow.document;

    // A. Thu gọn Sidebar khi tương tác bản đồ
    ...
    // B. Smooth Compass rotation (Bộ lọc làm mượt góc xoay la bàn)
    let currentHeading = 0;
    let targetHeading = 0;

    function renderSmoothHeading() {
        let diff = targetHeading - currentHeading;
        while (diff < -180) diff += 360;
        while (diff > 180) diff -= 360;

        currentHeading += diff * 0.15; // Hệ số mượt

        const arrow = pDoc.querySelector('#live-user-marker');
        if (arrow) {
            arrow.style.transform = `rotate(${currentHeading}deg)`;
        }
        requestAnimationFrame(renderSmoothHeading);
    }
    requestAnimationFrame(renderSmoothHeading);
    ...
    // C. Realtime High-Precision GPS Tracker
    let leafletMap = null;
    let userMarker = null;
        ...
        if ("geolocation" in navigator) {
            navigator.geolocation.watchPosition(
                (pos) => {
                    ...
                    // Cập nhật thẻ hiển thị tọa độ Sidebar
                    const statusEl = pDoc.querySelector('#gps-status');
                    if (statusEl) {
                        statusEl.innerText = `${lat.toFixed(5)}, ${lon.toFixed(5)}`;
                    }

                    // Tìm Leaflet Instance để move marker & pan camera trực tiếp trên màn hình
                    if (!leafletMap) {
                        const mapEl = pDoc.querySelector('.folium-map');
                        if (mapEl && mapEl._leaflet_id) {
                            for (let key in mapEl) {
                                if (key.startsWith('_leaflet_id')) {
                                    leafletMap = window.parent.L.index ? window.parent.L.index : null;
                                }
                            }
                        }
                    }

                    // Cập nhật vị trí URL param để dùng khi tính lộ trình OSRM
                    const currentUrl = new URL(pWindow.location.href);
                    const oldLat = currentUrl.searchParams.get('lat');
                    if (!oldLat || Math.abs(lat - parseFloat(oldLat)) > 0.0001) {
                        currentUrl.searchParams.set('lat', lat);
                        currentUrl.searchParams.set('lon', lon);
                        pWindow.history.replaceState({}, '', currentUrl);
                    }
                },
                (err) => {
                    const statusEl = pDoc.querySelector('#gps-status');
                    if (statusEl) statusEl.innerText = "Bật GPS điện thoại!";
                },
                {
